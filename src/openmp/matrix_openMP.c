...
void MatrixProcessing_nextState_openMP(cell_type *currentStateMatrix, cell_type* nextStateMatrix, unsigned int rows, unsigned int columns, double covid_power){
    ...
    #pragma omp parallel for collapse(2) schedule(static, __NUM_OF_THREADS__) private(rowIndex_1, columnIndex_1, rowOffset, currentStateCell, nextStateCell,contagiousCellsProportion, neighbors, rowNeighbor_1_index, rowNeighbor_2_index, rowNeighbor_3_index)
        //Process Matrix
